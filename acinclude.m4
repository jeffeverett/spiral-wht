...
[
AC_REQUIRE([AC_PROG_CC])
AC_CACHE_CHECK([whether ${CC-cc} accepts $1], [ac_cv_$2],
[echo 'void f(){}' > conftest.c
dnl catch cc WorkShop compiler warning for -native
if test "$CC" = cc && test -n "`cc -V 2>&1 | grep 'WorkShop 6'`"; then
...
if test "$ac_test_CFLAGS" != "set"; then
  ...
  *linux*) ;;
  ...
  if test $ac_cv_prog_gcc = yes; then
     CFLAGS="-fomit-frame-pointer -fopenmp"
     PCFLAGS=$CFLAGS
  fi
  ...
  if test "$GCC" = "yes"; then
    ...
    case "${host_cpu}" in
    i586*)  
      ACX_CHECK_CC_FLAGS(-mcpu=pentium,cpu_pentium,
      [CPU_FLAGS=-mcpu=pentium],
      [ACX_CHECK_CC_FLAGS(-mpentium,pentium,
        [CPU_FLAGS=-mpentium])])
      ;;
    i686*)
      ACX_CHECK_CC_FLAGS(-mcpu=pentiumpro,cpu_pentiumpro,
...
[
  ...
  AC_MSG_NOTICE([pcflags: ${PCFLAGS}])
        
  printf '#include <omp.h>\nvoid main(){omp_set_num_threads(2);}' > confptest.c
